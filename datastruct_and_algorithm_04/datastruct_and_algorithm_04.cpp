...
//方案
// 黑名单过滤
// 黑名单来源可以收集已公开信息，或用户主动标记，当标记超过阈值则加入黑名单
// 可以用布隆过滤器存储黑名单
// 另外利用网络通信，存储在服务端，接口请求是否拦截
// 
// 基于规则过滤
// 短信内容，包含非法词汇
// 发送号码是群发号码
...
//分析
// 相似用户
// 用户喜爱音乐的标签，与其他人标签的匹配程度，超过阈值，则看作偏好相似
// 另外，喜爱程度可通过单曲循环、分享、收藏、搜索、听完、没听过、跳过的级别加权
// 加权后，匹配程度相当于向量距离计算——欧几里得距离
// 
// 相似歌曲
// 根据歌曲特征值，同样是分析标签（机器标记或人工标注）
// 同样根据向量，计算距离，相近则推荐
//
...
//索引
//
//问题引入：对比mysql依赖B+，redis这种kv数据库索引如何实现
// 
//索引的需求
// 功能性需求
//	数据是否格式化；mysql就是结构化，对于非结构的数据（网页），可以进行预处理提取关键词
//	数据是静态还是动态；没有增删改，则只关注查询效率，动态数据还需要动态修改索引
//	索引放在内存还是硬盘；数据量大时，可能不得不放在硬盘，或者一部分存内存
//	单值还是区间查找；
//	单关键字还是多关键字；
// 非功能性
//	索引对空间消耗不能过大，如果超过了原始数据，那索引的作用就降低了
//	维护成本，动态更新索引的复杂度不能过高
//
//常用数据结构
// 散列表、红黑树、跳表、B+树 支持动态数据
// 位图、布隆过滤器 做辅助索引
// 数组 静态数据
// 
// 键值数据库，如redis，一般使用散列表，并且在内存中
// ext文件系统，对磁盘块索引，用的红黑树，也适合于内存索引
// B+更适合存储在磁盘中的索引，因为多叉树高度一般低于红黑树，磁盘IO次数会少一些，mysql、oracle都是B+树
// 跳表可以领过调整索引节点，是redis中有序集合的实现方式
// 布隆过滤器内存占用小，对于接受一定误判率的场景，可以作为辅助（将磁盘数据的索引加载到过滤器，先经过布隆，不存在的，就不读磁盘了）
//

//
//并行算法
// 
//问题引入：当算法无法继续优化，如何继续提升效率
// 
//并行排序
// 对大小8GB的数据排序（设定上内存足够）
// 常用O(nlogn)的三种排序方式，归并、快速、堆排序
// 
// 扫描一遍数据，划出数据范围，分割成n个区间
// 将数据对应到n个区间中，并开启n个线程并行排序
// 全部完成后整个区间都有序了
// 属于分治
// 
// 不同场景不同需求
// 如果1T数据，内存不足，则性能瓶颈可能由算法效率改为了磁盘IO性能问题
// 
//并行查找
// 存储在散列表中的数据越来越多时，扩容性能消耗较大
// 可以将数据分割成n份，每份分别建立散列表
// 查询时，就开启n个线程，同时在这些散列表中查询
// 添加数据时，可以选择装载因子最小的，减少散列冲突
// 
//并行字符串匹配
// 可将大字符串分割后，分别匹配模式串
// 另外，在分界处，取前后一些字符处理一下填补匹配的空缺
// 
//并行搜索
// 普通广度优先搜索是使用一个队列记录遍历到的节点
// 并行改造后，用两个队列完成
// 多线程并行处理队列A顶点，扩展到的顶点存队列B
// A处理完，并行处理B，扩展出来的顶点存在A
// 两个队列循环使用，提高效率（两个队列解决了遍历层级的同步问题）
//

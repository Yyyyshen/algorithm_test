...
// 
//分析
// 思路是 记录已经爬取的网页，爬取新网页时，先在已爬取列表中搜索，不存在才继续爬取并记录
// 
// 需要的操作是 添加和查询 效率需要尽量高
// 先选择一种数据结构
// 散列表、红黑树、跳表都满足快速插入和查找
// 
// 看内存 假设存储10亿条url 每条url平均64字节
// 散列表存储 大约需要 60GB 加上装载因子的影响（链表法额外需要链表指针）则最终实际内存可能超过100G
// 需要使用分治思想，分布到多台机器存储；但毕竟还是消耗太大了
// 并且，虽然大O时间复杂度为1，但实际上，哈希定位链表后还需要对比URL，也是一种耗时操作
// 另外，链表节点内存中不连续，CPU缓存不友好
// 
// 引出 布隆过滤器
// 基于 位图
// 
//位图最基本应用
// 在1000w个范围在1~1亿的整数，查找某个数是否在这些数中
// 可以申请一个足够大（1亿大小）的数组，将一千万个数的值作为下标，放入数组，对应位置标记1，其他位置为0
// 检查某整数是否在其中时，直接找数组下标为该整数的位置，看是否是1，是则存在
// 而是否存在只需要0和1来标记，对于0和1其实并不需要使用一个基本类型，即使是一个1字节的char也是浪费的
// 可以再将一字节拆成8位，这样一字节就可以表示八个数值是否存在了
class bitmap
{
public:
	bitmap(int bits)
		: nbits(bits)
	{
		bmp = new char[nbits / 8 + 1];//根据位数构建位图
	}
	~bitmap()
	{
		delete[] bmp;
...
// 1000w数据真实存储下来，每个占4字节，则大概需要40M内存，使用位图的话，需要1亿个二进制bit，大约是12M
// 如果数据量增长10倍，真实存储也是成倍增长，而只要数据范围不变，位图的内存占用是恒定的
// 反过来讲，数据量不变，数据范围扩大十倍，则4字节整型范围足够，内存消耗不变，但位图消耗扩大十倍为120M
// 布隆过滤器就是为了解决这样的情况，对位图进行改进
// 
//布隆过滤器
// 利用哈希，和散列表差不多的思想，将数据范围1~10亿的数据，通过哈希，落到1~1亿
// 
// 例如将数据对一亿求模，最简单的满足数据缩小10倍
// 但哈希冲突存在，比如1和1亿零1两个数字，哈希后都是1
// 布隆过滤器使用了一组哈希，降低冲突概率
// 
// 例如K个哈希函数，结果记为X1，X2，...XK
// 那么将这些数作为位图下标，都设置为1，也就是用K个二进制位表示一个数存在
// 查询某个数时，检查K个哈希函数的值Y1，Y2，...YK为下标的位置，如果都在位图中为1，则数存在
// 有一个是0则不存在
// 
// 依旧存在问题是，数字标记的位很多时，可能K个哈希正好都标记在了其他已经覆盖的位置
// 也就是查询存在情况会有误判（不存在则是一定不存在，没有误判）
// 还需要通过调整哈希的个数、位图的大小、存储数字的个数之间的比例
// 将误判概率降低
// 
// 尽管存在误判，其实，对于实际应用的影响来说已经非常小了
// 对于爬虫问题，偶尔一个URL误判为已爬取，在大量的数据中来看，是可以接受的，但换取的资源节约和效率是更大的
// 10亿个URL本来需要近100G内存，使用位图，用一个10倍范围（增加富裕位以减小误判率）的位图存储，100亿bit也才1.2G
// 并且，数组存储内存连续，内存密集型的读取，CPU缓存友好的话速度会更快
// 
//应用例
// 一亿整数，范围1到10亿，排序
// 遍历一遍添加到10亿bit位图，再依次读一遍就是有序的了，只需要120M内存（前提是不考虑重复数字）
// 用数组存1亿个整数再快排也可以解决，但需要400M内存
//

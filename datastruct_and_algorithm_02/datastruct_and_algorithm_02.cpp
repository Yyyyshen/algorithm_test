...
// Brute Force，暴力匹配
// 从长度为n的主串中匹配长度为m的模式串
// 则检查起始位置为 0，1，2...n-m，且长度为 m 的 n-m+1 个子串，看是否有匹配的
// 
// Java的indexOf就是BF算法，虽然时间复杂度很高为 O(n*m)
// 但实际开发中，用于匹配的两个字符串通常不会很长
// 每次匹配中，只逐个匹配直到遇到不能匹配的字符即可，不会每次都匹配m个
// 并且思想简单，不易出错
// 所以大部分时间够用
// 
//RK
// Rabin-Karp，相当于升级版BF
// 将主串中的 n-m+1 个子串分别求哈希值，然后逐个与模式串哈希值比较
// 减少了逐个比较字符的过程
// 
// 但如果哈希过程遍历了每个字符，虽然比较效率高了，算法整体上效率并没有提高
// 所以需要特殊设计哈希算法
//	匹配的字符集中包含K个字符，则使用K进制数表示一个子串，将K进制数转换为十进制数作为哈希值
// 
// 例如，匹配小写字母，a~z为26个字母
// 那么用二十六进制表示一个字符串，a表示0，b表示1，以此类推
// cba = c * 26 * 26 + b * 26 + a * 1 （类比十进制：321 = 3*10*10 + 2*10 + 1*1）
// 
// 利用这个算法的同时，在计算每个子串时，由于每次只移动一位，所以有m-1位是重合的，就可以快速计算出每个子串的值
// 进一步优化，将26^(m-1)的每个结果先计算好放入数组，需要乘的时候直接以次方数作为下标访问，还能继续提高效率
// 
// 时间复杂度O(n)
// 
// 此方法的哈希算法是没有散列冲突的，一个字符串对应一个K进制数，一定是唯一的（类似一就是1，一百就是100）
// 但还有一个问题，以进制来表示一个字符，每个字符计算时都是与进制指数相乘（14个z基本就达到long上限了），很容易溢出
// 
// 还需要继续优化，设计一种更简单的方法，例如字母对应数字相加，但可想而知，只要字母相同但顺序不同的字符串就会有哈希冲突
// 哈希冲突概率高其实并不是问题
// 只需要匹配到哈希值相等是，再真实逐字对比一次子串即可（我理解是，哈希本质上是缩小比较范围
// （Java中的equals函数中就是包含hashcode，这也是为什么重写equals一定要重写hashcode函数）
// 
